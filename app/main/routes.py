...
def index():
    ...
    events_today = Event.query.filter(Event.datetime >= today).filter(Event.datetime < today + timedelta(days=1)).order_by(Event.datetime).all()
    events_this_week = Event.query.filter(Event.datetime >= today + timedelta(days=1)).filter(Event.datetime < today + timedelta(days=7)).order_by(Event.datetime).all()
    events_later = Event.query.filter(Event.datetime >= today + timedelta(days=7)).order_by(Event.datetime).all()
    return render_template('index.html', title='Home Page', events_today=events_today, events_this_week=events_this_week, events_later=events_later)
...
def create_event():
    ...
    if form.validate_on_submit():
        ...
        event = Event(user_id=current_user.id, location_id=location_id, datetime=date_time, info=form.info.data)
        event.add_participant(current_user)
        db.session.add(event)
        db.session.commit()
        flash('Your event is now added! {} at {}'.format(event.datetime.strftime("%A %d %B, %H:%M"), event.location.name))
        if new_location:
            return redirect(url_for('main.add_location', location_id=new_location.id))
        return redirect(url_for('main.index'))
    return render_template('create_event.html', title='Create Event', form=form, locations=locations)
...
def delete_event(event_id):
    ...
    if event.participants != [current_user] and event.participants != []:
        flash('Other people said they would join, please make sure they know the event is cancelled')
...
def events_today():
    today = datetime.today().replace(hour=0, minute=0, second=0, microsecond=0)
    events_today = Event.query.filter(Event.datetime >= today).filter(Event.datetime < today + timedelta(days=1)).all()
